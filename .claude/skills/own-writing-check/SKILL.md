...
- After `/condense-to-overleaf` (which should now output to drafts, not directly to .tex)
...
($OL is defined in your project's CLAUDE.md or equivalent config — typically the Overleaf sync directory.)
...
[Details from manuscript-critic agent]

[If violations:]
Action needed: Revert violations? (User decides)
```
