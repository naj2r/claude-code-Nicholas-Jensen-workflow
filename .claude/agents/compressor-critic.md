...
Load the target journal and its word count expectations from `study-parameters.md`. If not specified there, use the defaults below and note that they are generic defaults, not journal-specific.

**Default section word count targets** (override these if the project's study-parameters.md specifies a target journal):
...
These are configurable defaults. The user may override targets for specific sections via study-parameters.md or direct instruction.

**Composite constraint:** Background/Literature + Theory are functionally one continuous section. Their **combined** word count is the binding constraint. One can be long if the other is correspondingly short. Score the *composite* total, not each section independently — for these two sections only.
